...
# 버전 및 업데이트 확인용
st.sidebar.success("✅ v8.0 - 모든 기능 통합 및 31일 수정완료")
...
if 'ins_df' not in st.session_state: st.session_state.ins_df = i_r
if 'excl_df' not in st.session_state: st.session_state.excl_df = e_r
if 'after_df' not in st.session_state: st.session_state.after_df = a_r
if 'excl_indiv_df' not in st.session_state: st.session_state.excl_indiv_df = ind_r
...
def create_monthly_pdf(target_row, month, worked_dates, h_map):
    ...
    pdf.cell(col_w[0], 10, "실시요일-수업시수", 1, 0, 'C'); pdf.cell(col_w[1], 10, f" {', '.join(dt)}", 1, 1, 'L'); pdf.ln(2)
    ...
    pdf.cell(cols[0]+cols[1]+cols[2], 10, "합계", 1, 0, 'C', fill=True); pdf.cell(cols[3], 10, f"{len(worked_dates)}일", 1, 0, 'C', fill=True); pdf.cell(cols[4], 10, f"{int(th)}시간", 1, 0, 'C', fill=True); pdf.cell(cols[5], 10, f"{int(tp):,}원", 1, 1, 'C', fill=True)
    return bytes(pdf.output())

# --- 3. 사이드바 (등록/수정) ---
with st.sidebar:
    st.header("👤 강사 관리")
    mode = st.radio("작업 선택", ["등록/수정", "공통제외 관리"])
    if mode == "등록/수정":
        sub = st.selectbox("구분", ["신규 등록", "수정/삭제"])
        if sub == "신규 등록":
            with st.form("add"):
                n = st.text_input("이름"); sj = st.text_input("과목", "통합과학"); cl = st.text_input("학급", "1학년"); r = st.number_input("정규단가", 25000); ra = st.number_input("방과후단가", 50000)
...
# --- 5. 상세 리포트 및 달력 (31일 짤림 방지 및 개인일정 관리) ---
if not st.session_state.ins_df.empty:
    ...
    # 개인 일정 관리 섹션
    st.subheader(f"📍 {target} 선생님 개인 일정 관리")
    ...
    st.subheader(f"📊 {target} 선생님 월별 리포트")
    cols = st.columns(2); t_reg, t_aft, t_att = 0, 0, 0

    for m in range(3, 13):
        with cols[(m-3)%2]:
            m_l = f"{m}월"; cal = calendar.monthcalendar(2026, m)
            st.write(f"#### 🗓️ {m_l}")
            ...
            with ac:
                ...
                if st.button(f"📄 {m}월 양식 PDF 생성", key=f"b_{m}"):
                    pdf = create_monthly_pdf(row, m_l, mw, hm)
                    fn = f"2026학년도 {m_l} {row.get('subject','')} 시간강사({target}선생님) 수업 현황.pdf"
                ...
                for w_idx, week in enumerate(cal):
                    ...
                    for i in range(5):
                        ...
                        if day == 0: html += '<td></td>'
                            ...
                            if d in work_dates: 
                                cl = "background:#90EE90; font-weight:bold;"; wh += hm.get(d.weekday(), 0); m_rc += 1
            ...
            m_rp, m_ap = m_rh * int(row['rate']), m_ah * int(row.get('rate_after', 50000))
    ...
    c1.metric("총 출근(정규기준)", f"{t_att}일"); c2.metric("정규 총 시수", f"{t_reg}h"); c3.metric("방과후 총 시수", f"{t_aft}h"); c4.metric("최종 예상 급여", f"{int((t_reg*row['rate'])+(t_aft*row.get('rate_after',50000))):,}원")

...
if 'ins_df' not in st.session_state:
    try:
        ...
        st.session_state.ins_df = pd.DataFrame(columns=['name', 'rate', 'rate_after', 'mon', 'tue', 'wed', 'thu', 'fri'])
...
def create_pdf(target_name, rate, rate_after, total_reg, total_after, total_pay, monthly_stats, attendance_total):
    ...
    pdf.cell(100, 8, txt=f"총 출근 일수: {attendance_total}일 | 총 시수: {int(total_reg + total_after)}시간", ln=True)
    ...
    if not st.session_state.ins_df.empty:
        for _, ins in st.session_state.ins_df.iterrows():
            ...
            if not st.session_state.excl_indiv_df.empty:
                ...
                indiv_ex = {date.fromisoformat(str(d)) for d in i_df[i_df['type']=='개인휴무']['date'].tolist()}
                indiv_add = {date.fromisoformat(str(d)) for d in i_df[i_df['type']=='추가출근']['date'].tolist()}
            
            hm_all = {0: int(ins['mon']), 1: int(ins['tue']), 2: int(ins['wed']), 3: int(ins['thu']), 4: int(ins['fri'])}
            curr = date(2026, 3, 1)
            while curr <= date(2026, 12, 31):
                is_workday = (curr.weekday() < 5 and curr not in all_excl_common and curr not in indiv_ex) or (curr in indiv_add)
                if is_workday:
...
if not st.session_state.ins_df.empty:
    ...
    st.subheader(f"📍 {target} 강사 개인 일정 관리 (추가 출근/개인 휴무)")
    col_ind1, col_ind2 = st.columns([0.6, 0.4])
    with col_ind1:
        with st.form(f"indiv_form_{target}"):
            in_date = st.date_input("날짜 선택")
            in_type = st.selectbox("구분", ["개인휴무", "추가출근"])
            in_note = st.text_input("사유")
            if st.form_submit_button("개인 일정 추가"):
                new_ind = pd.DataFrame([{"name": target, "date": in_date.isoformat(), "type": in_type, "note": in_note}])
    ...
    for m in range(3, 13):
        with cols[(m-3)%2]:
                    ...
                    for i in range(5):
                        ...
                        if day == 0: html += '<td></td>'
                            ...
                            if d in work_dates: 
                                ...
                                if d in indiv_adds: cls = "background:#add8e6; font-weight:bold;" # 추가출근은 하늘색
                            elif d in tips: 
            ...
            st.info(f"**💰 {m}월 통합: {(m_rp + m_ap):,}원**\n- 정규: {m_rp:,}원 ({int(m_rh)}h) | 방과후: {m_ap:,}원 ({int(m_ah)}h)\n- 출근: {m_att}일 / 통합 시수: {int(m_rh+m_ah)}h")
    ...
    try:
        ...
        st.caption("PDF 생성 대기 중 (font.ttf 필요)")

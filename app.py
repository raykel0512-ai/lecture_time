...
def create_monthly_pdf(target_row, month, worked_dates, h_map):
    ...
    h_labels = ["연번", "날짜", "요일", "수업시수", "강사료(원)", "비고"]
    for i, h in enumerate(h_labels): pdf.cell(cols[i], 10, h, 1, 0, 'C', fill=True)
    pdf.ln()
    ...
    pdf.cell(cols[0]+cols[1]+cols[2], 10, "합계", 1, 0, 'C', fill=True); pdf.cell(cols[3], 10, f"{len(worked_dates)}일", 1, 0, 'C', fill=True); pdf.cell(cols[4], 10, f"{int(total_h)}시간", 1, 0, 'C', fill=True); pdf.cell(cols[5], 10, f"{int(total_pay):,}원", 1, 1, 'C', fill=True)
    ...
    if mode == "등록/수정":
        ...
        if sub == "신규 등록":
            with st.form("add"):
                ...
                subj = st.text_input("담당 과목", "통합과학"); cls_i = st.text_input("학급", "1학년"); r = st.number_input("정규", 25000); ra = st.number_input("방과후", 50000)
...
col_main_ex, col_main_bud = st.columns([0.65, 0.35])
with col_main_ex:
...
with col_main_bud:
    ...
    if not st.session_state.ins_df.empty:
        for _, ins in st.session_state.ins_df.iterrows():
            ...
            grand_total += int(t_aft[['w1','w2','w3','w4','w5','w6']].sum().sum() * ins.get('rate_after', 50000))
...
if not st.session_state.ins_df.empty:
    ...
    with c_i2:
        ...
        if st.button("개인 일정 저장"):
            ...
            edited_ind['name'] = target; st.session_state.excl_indiv_df = pd.concat([others, edited_ind], ignore_index=True); conn.update(worksheet="Exclusions_Indiv", data=st.session_state.excl_indiv_df); st.rerun()

    # 데이터 준비
    cur_aft_df = st.session_state.after_df[st.session_state.after_df['name'] == target].copy().reset_index(drop=True)
    if cur_aft_df.empty: cur_aft_df = get_initial_after_df(target)
    for c in ['w1','w2','w3','w4','w5','w6']:
        if c not in cur_aft_df.columns: cur_aft_df[c] = 0
    ...
    for _, ex in target_ind_ex.iterrows():
        ...
        else: ind_adds.add(d); tips[d] = f"[추가] {ex['note']}"
    work_dates = [d for d in [date(2026,3,1) + timedelta(n) for n in range(306)] if (d.weekday() < 5 and d not in tips and d not in ind_adds and hm.get(d.weekday(), 0) > 0) or (d in ind_adds)]
        ...
        with cols[(m-3)%2]:
            ...
            with aft_c:
                ...
                for i in range(num_weeks):
                    val = int(cur_aft_df.at[r_idx, f'w{i+1}']) if f'w{i+1}' in cur_aft_df.columns else 0
                    w_in = st.number_input(f"{m}월 {i+1}주", value=val, step=1, key=f"w{i+1}_{target}_{m}")
                    wa.append(w_in); cur_aft_df.at[r_idx, f'w{i+1}'] = w_in
                    ...
                    for i in range(5):
                        ...
                        if day == 0: html += '<td></td>'
                            ...
                            if d in work_dates: 
                                ...
                                if d in ind_adds: cls = "background:#add8e6; font-weight:bold;"

...
def create_pdf(target_name, rate, rate_after, total_reg, total_after, total_pay, monthly_stats, attendance_total):
    ...
    pdf.cell(100, 8, txt=f"총 출근: {attendance_total}일 | 총 시수: {int(total_reg + total_after)}시간", ln=True)
    pdf.cell(100, 8, txt=f"총 예상 급여액: {int(total_pay):,}원", ln=True); pdf.ln(5)
    
    # 헤더
    pdf.cell(12, 10, "월", 1, 0, 'C'); pdf.cell(12, 10, "출근", 1, 0, 'C')
    pdf.cell(18, 10, "정규h", 1, 0, 'C'); pdf.cell(18, 10, "방과후h", 1, 0, 'C')
    pdf.cell(32, 10, "정규급여", 1, 0, 'C'); pdf.cell(32, 10, "방과후급여", 1, 0, 'C'); pdf.cell(32, 10, "월지급액", 1, 1, 'C')
    
    for m in monthly_stats:
        pdf.cell(12, 10, m['월'], 1, 0, 'C'); pdf.cell(12, 10, str(m['출근']), 1, 0, 'C')
        pdf.cell(18, 10, str(m['정규h']), 1, 0, 'C'); pdf.cell(18, 10, str(m['방과후h']), 1, 0, 'C')
        pdf.cell(32, 10, m['정규급여'], 1, 0, 'R'); pdf.cell(32, 10, m['방과후급여'], 1, 0, 'R'); pdf.cell(32, 10, m['급여'], 1, 1, 'R')
...
if not st.session_state.ins_df.empty:
    ...
    for m in range(3, 13):
        with cols[(m-3)%2]:
            ...
            # 출근 및 통계 계산
            aft_weeks_extra = sum([1 for wi, wv in enumerate([w1,w2,w3,w4,w5]) if wv > 0 and not any(date(2026,m,d) in monthly_reg_att_days for d in cal[wi] if d != 0)])
            ...
            **💰 {m}월 통합 급여: {m_total_pay:,}원**  
            ...
            - 출근: {m_att}일 / 총 시수: {int(m_reg_h+m_aft_h)}h
            """)
    ...
    # 저장 및 통계
    st.divider()
    if st.button(f"💾 {target} 강사 시수 저장"):
    ...
    c4.metric("최종 급여 합계", f"{int((total_reg_h*ins_row['rate'])+(total_aft_h*ins_row.get('rate_after',50000))):,}원")
